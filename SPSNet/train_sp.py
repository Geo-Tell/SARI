...
def parse_args():
    ...
    parse.add_argument('--lrStart2',default=3e-4,type=float)
...
def train(verbose=True, **kwargs):
    ...
    ds2 = LULCDataset(cfg, args, args.Trainset, 'train_sp')
    if args.local_rank <= 0:
    ...
    n_min = 200*200
    criteria = OhemCELoss(thresh=cfg.ohem_thresh, n_min=n_min, ignore_lb=cfg.ignore_label,class_weight=ds1.class_weights).cuda()
        ...
        if args.local_rank <= 0:
            ...
            if val_mIOU > best_mIoU:
                ...
                torch.save(net.state_dict(),
                        os.path.join(args.resPath, 'best.pth'))
            logger.info('=> saving checkpoint to {}'.format(
                args.resPath + '/checkpoint.pth.tar'))
            torch.save({
                'epoch': epoch+1,
                'best_mIoU': best_mIoU,
                'omit_list': omit_list,
            ...
            }, os.path.join(args.resPath,'checkpoint.pth.tar'))
